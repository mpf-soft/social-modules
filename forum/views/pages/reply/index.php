...
<?php /* @var $this mpf\modules\forum\controllers\Reply */ ?>
...
<?= \app\components\htmltools\Page::get()->title(\mpf\web\helpers\Html::get()->link($this->updateURLWithSection(['home', 'index']), $this->forumTitle)
    ...
    . " " . \mpf\modules\forum\components\Config::value('FORUM_PAGE_TITLE_SEPARATOR') . " "
    . $reply->author->name . " reply to " . $thread->title); ?>
...
<div class="forum-page <?= $this->forumPageTheme; ?>">
    ...
    <?php $this->displayComponent("pagelist", [
        ...
        'visibleElements' => count($reply->replies),
        ...
        <?php foreach ($replies as $reply) { ?>
            <tr class="forum-reply  forum-group-class-<?= $reply->authorGroup->html_class; ?>">
                ...
                <td class="forum-reply-content">
                    ...
                    <?php } ?>
                    <?php $this->display("../thread/_replies", ['reply' => $reply, 'level' => 2]); ?>
                </td>
            </tr>
        <?php } ?>
