...
class ModelHelper extends Object {
     ...
     * @return string
     */
    public static function getUserIconURL($icon) {
        if ('http' == substr($icon, 0, 4)) {
            return $icon;
        }
    ...
    public static function createSubscription($name, $type) {
        ...
        if (!Type::findByAttributes(['name' => $type])) {
            $typeModel = new Type();
            $typeModel->name = $type;
            $typeModel->title = self::getDefaultTitleForType($originalType);
            $typeModel->description = self::getDefaultDescriptionForType($originalType);
    ...
    protected static function getDefaultTitleForType($type) {
        switch ($type) {
            case "thread.moved":
                return "Forum thread moved";
            case "thread":
                return "New/edited forum reply";
            default:
                return "not-set";
        }
    }

    protected static function getDefaultDescriptionForType($type) {
        switch ($type) {
            case "thread.moved":
                return "Automatically generated by the forum module! Options: {\$admin}, {\$title}, {\$newCategory}, {\$threadId}, {\$adminId}";
            case "thread":
                return "Automatically generated by the forum module! Options: {\$threadTitle}, {\$action}, {\$threadId}, {\$userName}, {\$userId}";
            default:
                return "Automatically generated by the forum module! Can be edited to change the messages sent.";
        }
    }
    ...
    protected static function getDefaultMessageForType($type) {
        switch ($type) {
            case "thread.moved":
                return "Your thread '{\$title}' was moved by {\$admin} to {\$newCategory}!";
            case "thread":
                return "{\$userName} {\$action} in {\$threadTitle}";
            default:
                return "not-set";
        }
    }
